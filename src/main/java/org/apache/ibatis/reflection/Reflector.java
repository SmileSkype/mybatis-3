 ...
 * 反射器，每个 Reflector 对应一个类。Reflector
 * 会缓存反射操作需要的类的信息，例如：构造方法、属性名、setting / getting 方法等等
 */
public class Reflector {
   ...
   * key: 属性名  value:方法参数类型
   */
  private final Map<String, Class<?>> setTypes = new HashMap<>();
  /**
   * 属性对应的getting方法的返回值类型的映射
   * key:属性名 value:返回值的类型
   */
  ...
  /**
   *  之前会调用 canControlMemberAccessible 方法 设置构造方法可以访问，避免是 private 等修饰符
   */
  ...
  // 解决 getting 冲突方法。最终，一个属性，只保留一个对应的方法。代码如下
    ...
    for (Entry<String, List<Method>> entry : conflictingGetters.entrySet()) {
      ...
      for (Method candidate : entry.getValue()) {
        ...
        if (candidateType.equals(winnerType)) {
          // 返回值类型不是boolean类型，应该在 getClassMethods 方法中，已经合并。所以抛出 ReflectionException 异常
        ...
        // 实际上就是 candidateType表示的范围是否比winner大或者相同
