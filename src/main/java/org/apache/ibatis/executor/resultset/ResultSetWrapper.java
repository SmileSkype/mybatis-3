...
/**
 * @author Iwao AVE!
 * java.sql.ResultSet的包装器,可以理解为ResultSet的工具类,提供给DefaultResultSetHandler使用
 */
public class ResultSetWrapper {
  ...
  /**
   * TypeHandler 的映射 KEY1：字段的名字 KEY2：Java 属性类型
   */
  private final Map<String, Map<Class<?>, TypeHandler<?>>> typeHandlerMap = new HashMap<>();
  /**
   * 有 mapped 的字段的名字的映射   KEY：{@link #getMapKey(ResultMap, String)}  VALUE：字段的名字的数组
   */
  ...
  public TypeHandler<?> getTypeHandler(Class<?> propertyType, String columnName) {
    ...
    // <2> 如果获取不到，则基于 propertyType + jdbcType 进行查找。
    if (handler == null) {
      // <2.1> 获得 JdbcType 类型
      JdbcType jdbcType = getJdbcType(columnName);
      // <2.2> 获得 TypeHandler 对象
      handler = typeHandlerRegistry.getTypeHandler(propertyType, jdbcType);
      // Replicate logic of UnknownTypeHandler#resolveTypeHandler
      // See issue #59 comment 10
      /**
       *  <3> 如果获取不到，则基于 javaType + jdbcType 进行查找
       *  其中，javaType 使用 classNames 中的类型。而 #resolveClass(String className) 方法，获得对应的类
        */
  ...
  private void loadMappedAndUnmappedColumnNames(ResultMap resultMap, String columnPrefix) throws SQLException {
     ...
     *  getMapKey(resultMap, columnPrefix) = resultMap.getId() + ":" + columnPrefix;
     *   }
      */
  ...
  /**
   * resultMap.mappedColumns  拼接 prefix 到 columnNames 原来的属性中
   *
