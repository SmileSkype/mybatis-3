...
public class TypeAliasRegistry {
  ...
  public <T> Class<T> resolveAlias(String string) {
    try {
      ...
      if (TYPE_ALIASES.containsKey(key)) {
        ...
        // <2.2> 其次，直接获得对应类 直接获取对应的类。所以，这个方法，同时处理了别名与全类名两种情况
        value = (Class<T>) Resources.classForName(string);
      }
      return value;
    } catch (ClassNotFoundException e) { // <2.3> 异常 最差，找不到对应的类，发生异常，抛出 TypeException 异常
  ...
  public void registerAlias(String alias, Class<?> value) {
    ...
    // issue #748
    // <1> 转换成小写 将别名转换成**小写**。这样的话，无论我们在 Mapper XML 中，写 `String` 还是 `string` 甚至是 `STRING` ，都是对应的 String 类型。
